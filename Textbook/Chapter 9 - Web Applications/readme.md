...
_Zooming into the body section_
...
```html
<div id="container">
  <div id="left-child">LEFT</div>
  <div id="right-child">RIGHT</div>
</div>
```
...
- The `ul` container is used to state that an unordered list is being created - list items must be included for the `ul` styling to take effect - we use the `li` to indicate a new time in our list.
  **OUTPUT**
...
<body style="margin: 0; padding: 0;">
  <div
    style="background-color: brown; display: flex; justify-content: space-evenly;"
  >
